...
def SIT_heparin_response(value, target_interval):
    """
    For monitering the PTT, PT and INR to check the effectiveness of heparin
    """
    ...
    for patient_idx in tqdm(range(len(Sepsis_data))):
        ...
        for step in range(lengths-1):
            state, action, reward, next_state, done = (
                ...
                # Note: 第一层关注患者的关键生理信号，因为需要先保证患者存活；
                #  第二层关注患者对肝素的response，用Platelets, PTT, INR来感知；第三层关注患者长期结局
                # Feature index: {Platelets: 12, RDW: 15, PTT: 16, INR: 17, Lactate: 21, SOFA: 35 / -1}
                ...
                LongTermOutcome[step+1] * args.terminal_coeff,  # Note: 不+1吃不到结局标签

                states[step + 1], dones[step+1]  # Note: 不+1吃不到terminal标签
            )
